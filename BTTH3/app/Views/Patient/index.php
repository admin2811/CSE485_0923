    ...
    <div class="container">
        ...
        <table id="myTable" class="table">
            ...
            <tbody>
                <?php
                if (isset($patient)) {
                    foreach ($patient as $pti) {
                ?>
                        <tr>
                            <td scope="row"><?= $pti->getId() ?></td>
                            <td><?= $pti->getTenBenhNhan() ?></td>
                            <td><?= $pti->getNgayKham() ?></td>
                            <td><?= $pti->getTrieuChung() ?></td>
                            <td><?= $pti->getIdBacSi() ?></td>
                            <td><a href="<?= DOMAIN. 'public/index.php?controller=patient&action=editForm&id='.$pti->getId() ?>"><i class="fas fa-edit"></i></a></td>
                            <td><a href="<?= DOMAIN. 'public/index.php?controller=patient&action=delete&id='.$pti->getId() ?>" class='delBtn'><i class="fas fa-trash"></i></a></td>
                        </tr>
                <?php
                    }
                } else {
