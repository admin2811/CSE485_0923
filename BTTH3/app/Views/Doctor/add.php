  ...
  <div class="container">
    ...
    <form action="<?= DOMAIN . '/public/index.php?controller=doctor&action=insert' ?>"  method="post" id="form-data">
      <div class="input-group flex-nowrap">
        ...
        <input type="text" class="form-control" name="tenBS"  aria-describedby="addon-wrapping">
      </div>
      <br>
      <div class="input-group flex-nowrap">
        <span class="input-group-text" id="addon-wrapping">Tên Chuyên Khoa</span>
        <input type="text" class="form-control" name="tenCK"  aria-describedby="addon-wrapping">
      </div>
      <div class="d-flex justify-content-end mt-5"> <!-- Sử dụng lớp d-flex và justify-content-end -->
        ...
        <a href="<?= DOMAIN .'/public/index.php?controller=class&action=index'?>" class="btn btn-warning">Quay Lại</a>
      </div>
    </form>
  </div>    
</body>
...
<script>
    $('#form-data').submit(function (e) {
        ...
        if (this.checkValidity()) {
            $.ajax({
                url:'http://localhost:8080/CSE485_0923/BTTH3/public/index.php?controller=doctor&action=insert',
                type: "POST",
                data: $(this).serialize(),
